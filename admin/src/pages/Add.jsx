...
const Add = ({token}) => {
  ...
  const handleSizePriceChange = (index, field, value) => {
    const updatePrices = prices.map((item, i) =>
      i === index
        ? { ...item, [field]: field === "size" ? value.toUpperCase() : value } : item)
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      formData.append("prices", JSON.stringify(prices)) // Array of size-price pairs
        ...
        <div>
          ...
          {prices.map((item, index) => (
            <div key={index} className='flex items-end gap-4 mt-2'>
              <input
              onChange={(e)=>handleSizePriceChange(index, "size", e.target.value)} value={item.size}
              type="text" placeholder='(S, M, L)' className='px-3 py-2 ring-1 ring-slate-900/10 rounded bg-white w-20' />
          ...
          <button onClick={addSizePrice} type='button' className='btn-secondary !rounded !text-xs flexCenter gap-x-2 mt-4 !px-3 !py-1'>
            <FaPlus /> Add Sizing
          </button>
        </div>
